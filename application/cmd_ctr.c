...
float auto_set_v[3];//三个数据为x，y，wz的数据，float类型
...
static cJSON *ctr_mode(cJSON *dat)
{
	...
	else if(strcmp(cJSON_GetObjectItem(dat,"mode")->valuestring,visual_m)==0)
	{
		  char * mode_str=cJSON_GetObjectItem(dat,"mode")->valuestring;
			usb_printf("%s\n",mode_str);
		  cJSON* date = cJSON_GetObjectItem(dat,"date");//解析数据
...
//		{
		  char color_str[32];
			sprintf(color_str, "\"robot_color\":%f,", robot_color_hero);
