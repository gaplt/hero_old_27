    ...
    if(chassis_move_to_vector->Sport_mode==remote_mode)
		{
				...
				rc_deadband_limit(chassis_move_to_vector->chassis_RC->rc.ch[CHASSIS_X_CHANNEL], vx_channel, 200);    //200
				rc_deadband_limit(chassis_move_to_vector->chassis_RC->rc.ch[CHASSIS_Y_CHANNEL], vy_channel, 200);
...
//	}
  ...
  if(switch_is_up(mode->chassis_RC->rc.s[s_left]) || gimbal_ctrl.gimbal_rc->mouse.press_r == 1)  //左上视觉模式
	{
//	 	gimbal_ctrl.gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_VISUAL;
		gimbal_ctrl.gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_GYRO;
		mode->chassis_mode = chassic_zero;
